...
if [ -f "${LOG_DIR}/node${NODE_NUMBER}.log" ]; then
    ...
    echo "Log file ${LOG_DIR}/node${NODE_NUMBER}.log does not exist."
    exit 1
fi

# Check if bootnodes.txt is populated
if [ ! -s "$BOOTNODES_FILE" ]; then
    echo "Bootnodes file $BOOTNODES_FILE is empty or not populated."
    exit 1
fi
