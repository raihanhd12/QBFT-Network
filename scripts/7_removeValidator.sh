...
BASE_IP_FILE="base_ip.txt"  # File to store the BASE_IP
...
get_base_ip() {
    if [ -f "$BASE_IP_FILE" ]; then
        ...
        echo "BASE_IP file not found. Exiting."
        exit 1
    fi
}
...
for i in $(seq 1 $EXISTING_NODE_COUNT); do
    ...
    RESPONSE=$(curl -s -m 10 -X POST --data '{"jsonrpc":"2.0","method":"qbft_discardValidatorVote","params":["'"${VALIDATOR_ADDRESS}"'"], "id":1}' http://${BASE_IP}:${RPC_PORT})
...
for i in $(seq 1 $EXISTING_NODE_COUNT); do
    ...
    RESPONSE=$(curl -s -m 10 -X POST --data '{"jsonrpc":"2.0","method":"qbft_proposeValidatorVote","params":["'"${VALIDATOR_ADDRESS}"'", false], "id":1}' http://${BASE_IP}:${RPC_PORT})
    if echo "$RESPONSE" | grep -q '"result":true'; then
...
if [ $VALIDATOR_VOTE_COUNT -ge $MAJORITY_COUNT ]; then
    ...
    jq --arg rpcUrl "http://${BASE_IP}:$((8544 + ${NODE_NAME##*-}))" 'del(.nodes[] | select(.rpcUrl == $rpcUrl))' ${CONFIG_FILE} > tmp.$$.json && mv tmp.$$.json ${CONFIG_FILE}
